...
function Spotify(){
   ...
   async function download(){
    try{
        const youtube = await fetch(`/api/testing?url=${url}`) 
        const data = await youtube.json()
        console.log(data.data.dl_url)
        const video = data.data.dl_url;

        if (video) {
            setLink(<a href={video} className='btn btn-danger ms-5' style={{ position: 'relative', top: '10rem' }} download>download</a>);
        } else {
